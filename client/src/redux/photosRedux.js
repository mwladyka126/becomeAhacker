...
export const getTrendingPhotos = ({ photos }) =>
  [...photos.data].sort((a, b) => (a.votes > b.votes ? -1 : 1)).slice(0, 4);
export const getPhotos = ({ photos }) => photos.data;
export const getPhoto = ({ photos }, id) =>
  photos.data.find((photo) => photo._id === id);
...
export const addPhotoRequest = (data) => {
  return async (dispatch) => {
    ...
    try {
      let res = await axios.post(`${API_URL}/photos`, data, {
        headers: {
...
export default function reducer(statePart = initialState, action = {}) {
  switch (action.type) {
    ...
    case VOTE_PHOTO:
      return {
        ...statePart,
        data: statePart.data.map((photo) =>
          photo._id === action.payload
